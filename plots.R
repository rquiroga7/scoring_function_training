...
binding_energy$y[binding_energy$surface == 1] <- with(binding_energy[binding_energy$surface == 1,], -1.0 * exp(-(x-0.1)^2) - 0.5 * exp(-(x - 2)^2) - 0.4 * exp(-(x + 2)^2))
...
binding_energy$y[binding_energy$surface == 3] <- with(binding_energy[binding_energy$surface == 1,], -0.6 * exp(-(x+0.7)^2) -0.9 * exp(-(x - 2)^2) - 0.5 * exp(-(x + 2)^2))
...
pes_plot4 <- ggplot(binding_energy %>% filter(surface!=2), aes(x = x, y = y, color = as.factor(surface))) +
...
#Add 
  ...
  pes_plot4
ggsave("pes_plot_2_colors.png", plot = pes_plot4, width = 6, height = 4, units = "in", dpi = 600)
...
binding_energy_ <- data.frame(x = pes$x, y = -pes$y)
#Now take binding energy, create a new column named surface with value 1, duplicate all rows with surface value 2 and 3
binding_energy1 <- binding_energy_ %>% mutate(surface = 1)
binding_energy2 <- binding_energy_ %>% mutate(surface = 2)
binding_energy3 <- binding_energy_ %>% mutate(surface = 3)
#binding_energy4 <- binding_energy_ %>% mutate(surface = 4)
#binding_energy <- bind_rows(binding_energy1, binding_energy2, binding_energy3, binding_energy4)
binding_energy <- bind_rows(binding_energy1, binding_energy2, binding_energy3)
...
pes_plot5b
ggsave("pes_plot_VS.png", plot = pes_plot5b, width = 6, height = 6, units = "in", dpi = 600)
...
binding_energy$y[binding_energy$surface == 1] <- with(binding_energy[binding_energy$surface == 1,], -0.6* exp(-(x-0.8)^2) - 1.3 * exp(-(x - 2.5)^2) - 1.0 * exp(-(x + 0.9)^2))
#Modify the y values for surface 2, adding three different gaussians but maintaining the y value for x=0
...
#Do the same for surface 3, with different gaussians but same y value for x=0 as surface 1
binding_energy$y[binding_energy$surface == 3] <- with(binding_energy[binding_energy$surface == 1,], -1.0 * exp(-(x+0.3)^2) - 0.5 * exp(-(x - 1.0)^2) - 1.5 * exp(-(x + 2)^2))
#Surface 4
#binding_energy$y[binding_energy$surface == 4] <- with(binding_energy[binding_energy$surface == 1,], -0.25 * exp(-(x-0)^2) - 0.3 * exp(-(x - 1)^2) - 0.6 * exp(-(x + 1.5)^2))
...
pes_plot6b
ggsave("pes_plot_VS2.png", plot = pes_plot6b, width = 6, height = 6, units = "in", dpi = 600)
